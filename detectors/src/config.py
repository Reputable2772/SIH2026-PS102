...
# Base directories anchored to repository root, with environment variable override
_default_base = Path(__file__).resolve().parent.parent.parent
BASE_DIR = Path(os.environ.get("MPLADS_BASE_DIR", str(_default_base))).resolve()
...
class StatisticalThresholds:
    ...
    ROUND_NUMBER_VOUCHER_STEP: float = (
        10000.0  # Step for round voucher clustering analysis (Reserved for future FIN-D7)
    )
