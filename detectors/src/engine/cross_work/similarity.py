...
class DuplicateWorkDetector(BaseDetector):
    ...
    def __init__(
        ...
        max_block_size: Optional[int] = 1000,
        max_findings: Optional[int] = None,
    ):
        super().__init__(code="SIM-D12", name="Potentially Duplicate Work", category=AnomalyCategory.NETWORK_SIMILARITY)
    ...
    def detect(self, df_works: pd.DataFrame, baseline_engine: Optional[object] = None) -> List[Finding]:
        ...
        for (state, cat), sub in grp:
            ...
            if self.max_block_size and len(sub) > self.max_block_size:
                sub = sub.head(self.max_block_size)
            ...
            # Vectorized candidate selection using upper-triangle mask
            i_indices, j_indices = np.where(np.triu(sim_matrix >= self.similarity_threshold, k=1))
