...
from src.data.lifecycle import LifecycleStage, WorkLifecycleReconstructor
...
def test_payment_status_and_penny_drop_invariants():
    ...
    comp = pd.DataFrame(
        columns=["WORK_RECOMMENDATION_DTL_ID", "ACTUAL_AMOUNT", "ACTUAL_END_DATE", "AVERAGE_RATING", "WORK_ID"]
    )
...
def test_canonical_identity_uniqueness_and_cross_house_collision_immunity():
    ...
    if parquet_path.exists():
        ...
        if "LETTER_NO" in df.columns:
            ...
            compound_key = (
                df["house"].astype(str)
                + "_"
                + df["WORK_RECOMMENDATION_DTL_ID"].astype(str)
                + "_"
                + df["LETTER_NO"].astype(str)
            )
    ...
    rec_ls = pd.DataFrame(
        [
            {
                "WORK_RECOMMENDATION_DTL_ID": colliding_id,
                "RECOMMENDATION_DATE": "2024-01-01",
                "RECOMMENDED_AMOUNT": 500000.0,
            }
        ]
    )
    sanc_ls = pd.DataFrame(
        [
            {
                "WORK_RECOMMENDATION_DTL_ID": colliding_id,
