...
def test_recurrence_tiny_portfolios_not_extreme():
    ...
    works = pd.DataFrame([
        {"WORK_RECOMMENDATION_DTL_ID": "T1", "ia_name": "TINY_IA", "STATE_NAME": "GOA", "IDA_NAME": "NORTH GOA", "SANCTION_AMOUNT": 100000.0},
        {"WORK_RECOMMENDATION_DTL_ID": "T2", "ia_name": "TINY_IA", "STATE_NAME": "GOA", "IDA_NAME": "NORTH GOA", "SANCTION_AMOUNT": 100000.0}
    ])
    prior = [
        Finding(
            finding_id="FT1", work_id="T1", work_rec_id="T1", detector_code="COMP-D1",
            detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.7, confidence=0.8,
            evidence={}, explanation="", next_review_action=""
        ),
        Finding(
            finding_id="FT2", work_id="T2", work_rec_id="T2", detector_code="COMP-D2",
            detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.8, confidence=0.8,
            evidence={}, explanation="", next_review_action=""
        )
    ]
...
def test_recurrence_large_portfolios_not_penalized_for_volume():
    ...
    for i in range(50):
        rid = f"GL_{i}"
        works.append({"WORK_RECOMMENDATION_DTL_ID": rid, "ia_name": "GOOD_LARGE_IA", "STATE_NAME": "UP", "IDA_NAME": "AGRA", "SANCTION_AMOUNT": 200000.0})
        if i < 5:
            prior.append(Finding(
                finding_id=f"F_GL_{i}", work_id=rid, work_rec_id=rid, detector_code="COMP-D1",
                detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.5, confidence=0.8,
                evidence={}, explanation="", next_review_action=""
            ))
    ...
    for i in range(50):
        rid = f"BL_{i}"
        works.append({"WORK_RECOMMENDATION_DTL_ID": rid, "ia_name": "BAD_LARGE_IA", "STATE_NAME": "UP", "IDA_NAME": "AGRA", "SANCTION_AMOUNT": 200000.0})
        if i < 40:
            prior.append(Finding(
                finding_id=f"F_BL_{i}", work_id=rid, work_rec_id=rid, detector_code="COMP-D1",
                detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.5, confidence=0.8,
                evidence={}, explanation="", next_review_action=""
            ))
    ...
    for a in range(10):
        for w in range(10):
            rid = f"BG_{a}_{w}"
            works.append({"WORK_RECOMMENDATION_DTL_ID": rid, "ia_name": f"PEER_IA_{a}", "STATE_NAME": "UP", "IDA_NAME": "AGRA", "SANCTION_AMOUNT": 200000.0})
            if w < 4:
                prior.append(Finding(
                    finding_id=f"F_BG_{a}_{w}", work_id=rid, work_rec_id=rid, detector_code="COMP-D1",
                    detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.5, confidence=0.8,
                    evidence={}, explanation="", next_review_action=""
                ))
    ...
    assert "GOOD_LARGE_IA" not in flagged_entities, "Large agency with low anomaly rate (10%) must NOT be penalized for volume"
...
def test_recurrence_missing_exposure_distinction():
    ...
    works = pd.DataFrame([
        {"WORK_RECOMMENDATION_DTL_ID": "M1", "ia_name": None, "STATE_NAME": "BIHAR", "IDA_NAME": "GAYA", "SANCTION_AMOUNT": 100000.0},
        {"WORK_RECOMMENDATION_DTL_ID": "M2", "ia_name": np.nan, "STATE_NAME": "BIHAR", "IDA_NAME": "GAYA", "SANCTION_AMOUNT": 100000.0},
        {"WORK_RECOMMENDATION_DTL_ID": "M3", "ia_name": "", "STATE_NAME": "BIHAR", "IDA_NAME": "GAYA", "SANCTION_AMOUNT": 100000.0},
    ])
    prior = [
        Finding(finding_id="FM1", work_id="M1", work_rec_id="M1", detector_code="COMP-D1", detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.5, confidence=0.8, evidence={}, explanation="", next_review_action=""),
        Finding(finding_id="FM2", work_id="M2", work_rec_id="M2", detector_code="COMP-D1", detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.5, confidence=0.8, evidence={}, explanation="", next_review_action=""),
        Finding(finding_id="FM3", work_id="M3", work_rec_id="M3", detector_code="COMP-D1", detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.5, confidence=0.8, evidence={}, explanation="", next_review_action=""),
    ]
    det = EntityRecurrenceDetector(min_anomalies=3, min_exposure=3)
    findings = det.detect(works, prior)
...
def test_recurrence_deterministic_and_reproducible():
    ...
    works = pd.DataFrame([
        {"WORK_RECOMMENDATION_DTL_ID": f"D_{i}", "ia_name": "STABLE_IA", "STATE_NAME": "KERALA", "IDA_NAME": "KOCHI", "SANCTION_AMOUNT": 300000.0}
        for i in range(10)
    ])
    prior = [
        Finding(finding_id=f"FD_{i}", work_id=f"D_{i}", work_rec_id=f"D_{i}", detector_code="FIN-D5", detector_name="", category=AnomalyCategory.FINANCIAL, severity=0.8, confidence=0.9, evidence={}, explanation="", next_review_action="")
        for i in range(8)
    ]
    det = EntityRecurrenceDetector(min_anomalies=3, min_exposure=3)
...
def test_recurrence_explainability_fields_integrity():
    ...
    works = pd.DataFrame([
        {"WORK_RECOMMENDATION_DTL_ID": f"E_{i}", "ia_name": "EXPLAIN_IA", "STATE_NAME": "ASSAM", "IDA_NAME": "JORHAT", "SANCTION_AMOUNT": 400000.0}
        for i in range(10)
    ])
    prior = [
        Finding(finding_id=f"FE_{i}", work_id=f"E_{i}", work_rec_id=f"E_{i}", detector_code="COMP-D2", detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.75, confidence=0.85, evidence={}, explanation="", next_review_action="")
        for i in range(9)
    ]
    det = EntityRecurrenceDetector(min_anomalies=3, min_exposure=3)
    findings = det.detect(works, prior)
    ...
    f = findings[0]
    ev = f.evidence
    for k in ["entity_type", "entity_name", "flagged_works_count", "total_portfolio_works", "raw_failure_rate", "shrunk_failure_rate", "peer_baseline_rate", "z_score", "p_value", "contributing_anomaly_types"]:
...
def test_recurrence_severity_and_confidence_distinct():
    ...
    for i in range(4):
        rid = f"SMALL_{i}"
        works.append({"WORK_RECOMMENDATION_DTL_ID": rid, "ia_name": "SMALL_IA", "STATE_NAME": "DELHI", "IDA_NAME": "EAST", "SANCTION_AMOUNT": 100000.0})
        prior.append(Finding(finding_id=f"FS_{i}", work_id=rid, work_rec_id=rid, detector_code="COMP-D1", detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.6, confidence=0.8, evidence={}, explanation="", next_review_action=""))

    for i in range(60):
        rid = f"LARGE_{i}"
        works.append({"WORK_RECOMMENDATION_DTL_ID": rid, "ia_name": "LARGE_IA", "STATE_NAME": "DELHI", "IDA_NAME": "EAST", "SANCTION_AMOUNT": 100000.0})
        if i < 30:
            prior.append(Finding(finding_id=f"FL_{i}", work_id=rid, work_rec_id=rid, detector_code="COMP-D1", detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.6, confidence=0.8, evidence={}, explanation="", next_review_action=""))

    # Add background peers with 25% failure rate
    for a in range(15):
        for w in range(10):
            rid = f"BG_{a}_{w}"
            works.append({"WORK_RECOMMENDATION_DTL_ID": rid, "ia_name": f"BG_{a}", "STATE_NAME": "DELHI", "IDA_NAME": "EAST", "SANCTION_AMOUNT": 100000.0})
            if w < 2:
                prior.append(Finding(finding_id=f"FB_{a}_{w}", work_id=rid, work_rec_id=rid, detector_code="COMP-D1", detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.6, confidence=0.8, evidence={}, explanation="", next_review_action=""))
...
def test_recurrence_no_silent_fallback_to_raw_k():
    ...
    for i in range(100):
        rid = f"SAFE_{i}"
        works.append({"WORK_RECOMMENDATION_DTL_ID": rid, "ia_name": "SAFE_IA", "STATE_NAME": "MP", "IDA_NAME": "BHOPAL", "SANCTION_AMOUNT": 100000.0})
        if i < 5:
            prior.append(Finding(finding_id=f"F_SAFE_{i}", work_id=rid, work_rec_id=rid, detector_code="COMP-D1", detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.5, confidence=0.8, evidence={}, explanation="", next_review_action=""))

    # Peers with 30% failure rate
    for a in range(5):
        for w in range(20):
            rid = f"P_{a}_{w}"
            works.append({"WORK_RECOMMENDATION_DTL_ID": rid, "ia_name": f"PEER_{a}", "STATE_NAME": "MP", "IDA_NAME": "BHOPAL", "SANCTION_AMOUNT": 100000.0})
            if w < 6:
                prior.append(Finding(finding_id=f"FP_{a}_{w}", work_id=rid, work_rec_id=rid, detector_code="COMP-D1", detector_name="", category=AnomalyCategory.COMPLIANCE, severity=0.5, confidence=0.8, evidence={}, explanation="", next_review_action=""))
