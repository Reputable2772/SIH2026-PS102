# Phase 1 — Analytical Foundation & Canonical Model Specification
**MPLADS Intelligence Engine (SIH PS102)**  
**Authoritative Input Baseline:** `Phase 0 Data Audit (MPLADS/eSAKSHI — 2026-09-21 snapshot)`  
**Status:** Canonical Analytical Specification  
**Document Version:** 1.1.0 (Incorporating Architectural Review & Epistemic Controls)  
...
### 3.1 Source Identity vs. Disambiguation
Phase 0 established the following empirical facts regarding entity identification:
1. **Core Source Key:** Within any single parliamentary chamber and normal e-SAKSHI operation, the backend tracks works using `WORK_RECOMMENDATION_DTL_ID`. Therefore, the canonical source identity is:
   $$\text{Source Identity} = (\text{HOUSE\_OF\_PARLIAMENT},\; \text{WORK\_RECOMMENDATION\_DTL\_ID})$$
2. **The Disambiguation Need:** In `mplads_rajya_sabha_recommended.csv`, exactly 46 sequence numbers appear twice (spanning 92 rows). These were proven in Phase 0 to represent distinct works initiated by different MPs in different states, arising from a migration collision between legacy un-prefixed (`NA-`) records and modern (`WS/MP...`) records.
3. **The Role of `LETTER_NO`:** `LETTER_NO` is an administrative dispatch reference from the MP office, not an intrinsic entity primary key. However, because each MP's dispatch letter is unique to that submission, `LETTER_NO` provides a 100.0% reliable **disambiguator** for the 46 legacy collisions.

### 3.2 Key Formulation and Cross-Table Propagation
To model this cleanly without pretending every table natively carries an artificial composite key:

```text
┌────────────────────────────────────────────────────────────────────────────────────────┐
│                               RELATIONAL WORK IDENTITY MODEL                           │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ Canonical Source Key:  (HOUSE_OF_PARLIAMENT, WORK_RECOMMENDATION_DTL_ID)               │
│ Disambiguation Key:    LETTER_NO (Applied at Recommendation layer where collisions occur)│
└────────────────────────────────────────────────────────────────────────────────────────┘
```
...
The analytical foundation defines 12 distinct entities organized to preserve native cardinalities:
...
| **Work** | **Sanction** | $1 \rightarrow 1$ (Opt.) | `(HOUSE, DTL_ID)` | Directly Observed | 33,567 recommendations lack sanction entity (pending). |
| **Work** | **Completion** | $1 \rightarrow 1$ (Opt.) | `(HOUSE, DTL_ID)` | Directly Observed | 45,704 completed assets; 55,807 active pipeline. Zero orphan completions. |
| **Work** | **Expenditure** | $1 \rightarrow \text{Many}$ (Opt.)| `(HOUSE, DTL_ID)` | Directly Observed | **Preserve 1:M relationship.** Multiple payment vouchers per work are never flattened. |
...
*Phase 1 Boundary:* No anomaly cutoffs or threshold flags are assigned to these durations in Phase 1. Threshold design and peer-group calibration belong strictly to Phase 2.
...
- **Engine Handling:** For sanctioned works without payments, assign `IA_NAME = 'NOT_OBSERVED_PRE_PAYMENT'`. Do **not** flag as administrative failure to assign an agency.
...
| **Missing Documentary Attachments** | 67.77% null in LS Sanctions; 26.16% null in LS Completed | Document scanning/upload backlog at district level. | Ingest `FILE_STATUS` and `ATTACH_ID` verbatim. | **Data-Quality Indicator:** Set `FLAG_DOCUMENT_ATTACHED = FALSE`. Evaluated as documentary compliance indicator, not financial irregularity. |
...
The following derived metrics will be computed by downstream analytical pipelines during Phase 2 and Phase 3:
...
| `COST_VARIANCE_ACTUAL_TO_SANC` | Work | Completion vs sanction | $\frac{\text{ACTUAL\_AMOUNT} - \text{SANCTION\_AMOUNT}}{\text{SANCTION\_AMOUNT}}$ | `ACTUAL_AMOUNT`, `SANCTION_AMOUNT` | If either is null or `ACTUAL_AMOUNT == 0`, returns `NULL`. |
...
| `VENDOR_MARKET_SHARE_DISTRICT` | Vendor | Vendor volume dominance | $\frac{\sum \text{FUND\_DISBURSED\_AMT}_{\text{Vendor, Dist}}}{\sum \text{FUND\_DISBURSED\_AMT}_{\text{Dist}}}$ | `FUND_DISBURSED_AMT`, `VENDOR_ID`, `IDA_NAME` | Evaluated within district; returns `NULL` if district total is 0. |

*Phase 1 Rule:* These derived fields are specified here for architectural alignment. They will not be calculated until Phase 2 and Phase 3 pipelines are executed.
...
**Phase 1 is complete. No unresolved modeling ambiguity remains that would impede Phase 2 (Baselines & Peer-Group Specification).**
