...
4. **Resolution via Dispatch Reference:** Including the MP's dispatch letter reference `LETTER_NO` completely eliminates all 46 duplicates:
   ```text
   K_work = (HOUSE_OF_PARLIAMENT, WORK_RECOMMENDATION_DTL_ID, LETTER_NO)
   ```
...
- Partitioned by chamber using `K_work`:
...
1. **Source Work Identity:**
   ```text
   Source Identity = (HOUSE_OF_PARLIAMENT, WORK_RECOMMENDATION_DTL_ID)
   ```
...
2. **Recommendation Disambiguation:**
   ```text
   Disambiguator = LETTER_NO (Administrative Dispatch Reference)
   ```
...
4. **Downstream Child Linkage:**
   ```text
   Downstream Join Key = (HOUSE_OF_PARLIAMENT, WORK_RECOMMENDATION_DTL_ID)
   ```
...
| Target Join Relationship | Left Entity | Right Entity | Operational Join Key | Cardinality | Empirical Match % (Left → Right) | Join Integrity Finding |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Recommendation → Sanction** | Recommended | Sanctioned | `(HOUSE, DTL_ID, LETTER_NO)` | 1 → 1 | 74.70% → **99.39%** | 100,896 matched works; 33,567 pending; 615 orphan sanctions. 0 cross-house collisions. |
| **Sanction → Completion** | Sanctioned | Completed | `(HOUSE, DTL_ID, LETTER_NO)` | 1 → 1 | 45.02% → **100.00%** | 45,704 completed assets; 55,807 sanctioned works without an observed completion record. Zero orphan completions. |
| **Sanction → Expenditure** | Sanctioned | Expenditures | `(HOUSE, DTL_ID)` | 1 → Many | 72.35% → **100.00%** | 73,448 sanctioned works have disbursements; 28,063 have no observed expenditure in snapshot. Zero orphan payments. |
| **Completion → Expenditure** | Completed | Expenditures | `(HOUSE, DTL_ID)` | 1 → Many | **99.78%** → 61.34% | 45,605 completed assets have expenditure records; 99 completed lack payment records. |
| **Completion → Expenditure (via WORK_ID)**| Completed | Expenditures | `WORK_ID` | N/A | **0.00%** → **0.00%** | **Incompatible datatypes** (integer sequence vs formatted string). Must NOT be used. |
| **Expenditure → Vendor** | Expenditures | Commercial Vendors | `VENDOR_ID` | Many → 1 | **100.00%** → **100.00%** | 30,839 unique vendors mapped across 111,935 transactions. 0% nulls. |
| **Expenditure → IA** | Expenditures | Implementing Agencies | `IA_NAME` | Many → 1 | **100.00%** → **100.00%** | 7,378 distinct executing agencies mapped. Observed exclusively on works with payments. |
| **MP → Allocations** | Recommendations | Allocations | `(HOUSE, MP_NAME, TENURE)` | Many → 1 | **100.00%** → 95.10% | 737 MPs matched (538 LS, 199 RS). 38 MPs in allocation ledger have 0 recommendations (5 LS, 33 RS). |
...
| `IN_PROGRESS_DISBURSING` | Sanctioned; active payments | Sanction observed, ≥ 1 payment voucher, not completed. | **27,843 works** |
...
3. **Additive Invariant for Expenditures:**
   ```text
   TOTAL_DISBURSED_AMT = SUM(FUND_DISBURSED_AMT) across vouchers for a single work
   ```
...
| :--- | :--- | :--- | :--- | :--- |
| **Rajya Sabha Sequence Collisions** | 46 duplicate IDs across 92 rows in RS Rec | Legacy migration sequence collision between `NA-` and `WS/MP...` records. | Disambiguated by `LETTER_NO` in `K_work`. | Key disambiguation resolves 100.0%. Both records preserved. |
...
| `TOTAL_DISBURSED_AMT` | Work | Cumulative disbursements | `SUM(FUND_DISBURSED_AMT)` across vouchers for work | `FUND_DISBURSED_AMT` | **Returns `NULL` if `HAS_EXPENDITURE = FALSE`.** |
| `VOUCHER_COUNT` | Work | Payment transaction count| `COUNT(Voucher)` across vouchers for work | `FUND_DISBURSED_AMT` | If 0 vouchers exist, returns `0`. |
| `VENDOR_COUNT` | Work | Number of distinct vendors| `COUNT(DISTINCT VENDOR_ID)` for work | `VENDOR_ID` | If 0 vouchers exist, returns `0`. |
...
| `FINANCIAL_UTILIZATION_RATIO`| Work | Sanction utilization rate| `TOTAL_DISBURSED_AMT / SANCTION_AMOUNT` | `TOTAL_DISBURSED_AMT`, `SANCTION_AMOUNT` | If `SANCTION_AMOUNT IS NULL`, `== 0`, or `TOTAL_DISBURSED_AMT IS NULL`, returns `NULL`. |
| `COST_VARIANCE_SANCTION_TO_REC`| Work | Sanction vs proposal drift| `(SANCTION_AMOUNT - RECOMMENDED_AMOUNT) / RECOMMENDED_AMOUNT` | `SANCTION_AMOUNT`, `RECOMMENDED_AMOUNT` | If either is null or `RECOMMENDED_AMOUNT == 0`, returns `NULL`. |
| `COST_VARIANCE_ACTUAL_TO_SANC`| Work | Completion vs sanction drift| `(ACTUAL_AMOUNT - SANCTION_AMOUNT) / SANCTION_AMOUNT` | `ACTUAL_AMOUNT`, `SANCTION_AMOUNT` | If `SANCTION_AMOUNT IS NULL`, `== 0`, or `ACTUAL_AMOUNT IS NULL`, returns `NULL`. When `ACTUAL_AMOUNT == 0`, evaluates to `-1.0` (-100% variance) with `FLAG_ZERO_COST_COMPLETION = TRUE`. |
| `DAYS_TO_SANCTION` | Work | Recommendation to sanction| `SANCTION_DATE - RECOMMENDATION_DATE` | `SANCTION_DATE`, `RECOMMENDATION_DATE` | If either date is null, returns `NULL`. |
| `DAYS_TO_FIRST_PAYMENT` | Work | Sanction to first voucher | `min(EXPENDITURE_DATE) - SANCTION_DATE` | `SANCTION_DATE`, `EXPENDITURE_DATE` | If either date is null, returns `NULL`. |
| `DAYS_TO_COMPLETION` | Work | Sanction to completion | `ACTUAL_END_DATE - SANCTION_DATE` | `ACTUAL_END_DATE`, `SANCTION_DATE` | If either date is null, returns `NULL`. |
| `DAYS_COMPLETION_TO_FINAL_PAYMENT`| Work | Completion to final voucher| `max(EXPENDITURE_DATE) - ACTUAL_END_DATE` | `ACTUAL_END_DATE`, `EXPENDITURE_DATE` | If either date is null, returns `NULL`. |
| `PAYMENT_SPAN_DAYS` | Work | Disbursement duration | `max(EXPENDITURE_DATE) - min(EXPENDITURE_DATE)` | `EXPENDITURE_DATE` | If < 2 vouchers exist, returns `0`. |
...
| `VENDOR_MARKET_SHARE_IDA` | Vendor | Vendor volume dominance | `SUM(FUND_DISBURSED_AMT for Vendor, IDA) / SUM(FUND_DISBURSED_AMT for IDA)` | `FUND_DISBURSED_AMT`, `VENDOR_ID`, `IDA_NAME` | Evaluated within Implementing District Authority (`IDA_NAME`) as available administrative proxy; returns `NULL` if IDA total is 0. |
...
| **D6** | **Cost Estimation & Sanction Drift Profiler** | **Phase 1** (Financial) | `RECOMMENDED_AMOUNT`, `SANCTION_AMOUNT`, `ACTUAL_AMOUNT` | Evaluates variance ratios `Sanction / Recommend` and `Actual / Sanction` for statistical outlier behavior. | **Coverage disaggregated by scope:** 100,896 rec → sanction pairs; 45,704 sanction → actual pairs; 45,497 full 3-stage comparisons (207 completions lack recommendation data). |
