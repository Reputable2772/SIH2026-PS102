...
- **Core Library (`detectors/src/`)**: Stateless, modular analytical modules for data processing, rule/statistical detection, network analysis, composite risk scoring, and machine learning. Usable in external scripts, automated workflows, or downstream services (`from detectors import MPLADSEngine`).
- **CLI Wrapper (`detectors/src/cli.py`)**: An interactive command-line interface for running audits, triage review, model training, and validation stress tests.
...
nix develop --command python3 -m detectors.src.cli --help
# Or use the Nix package directly:
nix run .#mplads-engine -- --help
```
...
```bash
# CLI text printout
python3 -m detectors.src.cli dossier 202625

# Self-contained interactive HTML export with AC-19 checklist
python3 -m detectors.src.cli dossier 202625 --html reports/dossier_202625.html --json reports/dossier_202625.json
```
*Governance Output:*
...
Run the complete 59-test regression and matrix suite:
```bash
nix develop --command pytest -v
# or:
pytest -v
```
- `detectors/tests/test_phase_0.py`: Normalization, lifecycle reconstruction, DQI, coverage matrix.
- `detectors/tests/test_phase_1.py`: Baselines, compliance (D1-D4), financial (D5-D8), execution (D9), agency (D11).
- `detectors/tests/test_phase_2.py`: Duplicate similarity (D12), agency monopolization (D13), vendor capture (D14), recurrence (D15), trends.
- `detectors/tests/test_phase_3.py`: Two-axis risk scoring, dossier builder, injection monotonicity, CAG benchmark recall.
- `detectors/tests/test_phase_4.py`: Leakage guard, observation window label builder, Isolation Forest, Gradient Boosting, model persistence.
- `detectors/tests/test_engine_facade.py`: Decoupled `MPLADSEngine` facade, dataset results, JSON/HTML exports.
- `detectors/tests/test_html_report.py`: HTML rendering for audit reports and individual case dossiers.
- `detectors/tests/test_detection_matrix.py`: Complete detection matrix across isolated archetypes, live canonical rows, and 50 randomly mutated synthetic rows.
- `detectors/tests/test_scraper.py`: Scraper security (path traversal), JSON parsing, retry/backoff logic, and circuit breaker.
